...

{
    ...
    "raw_alert_payload": "the original payload exactly as received in dict format, without any modifications or omissions",
...
def triage_node(state: dict) -> dict:
    ...
    print(
        "[TRIAGE COMMANDER]"
        f"Analyzing raw payload: {raw_alert_payload}"
    )

    llm = build_triage_llm()
    messages = [
        SystemMessage(content=PROMPT),
        HumanMessage(content=str(raw_alert_payload))
    ]
    ...
    print(
        "[TRIAGE COMMANDER]"
        f"LLM have structured the alert payload: {str(parsed_data)}"
    )
